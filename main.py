...
if prompt:
    with st.spinner("Generating response.."):
        generated_response = from_llm(query=prompt)
        source = list(set([i.metadata['source'] for i in generated_response['source_documents']]))
...
if st.session_state:
    for question, answer in list(reversed(list(zip(st.session_state["user_input_history"],st.session_state["bot_answer_history"])))):
